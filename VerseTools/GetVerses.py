...
class GetVerses:
    ...
    def __init__(self, lang=None, references=None):
        # Initialize class with language and references
        if lang is None:
            with open(os.path.dirname(__file__)+"/resources/preferences.json",'r') as rf:
                pref = json.load(rf)
            print("No language was specified. Choosing the default language: {}".format(pref["default_language"]))
        ...
        if references is None:
            sys.exit("No references provided - Please provide a reference to retrieve a verse")
        ...
        self.__connect__()
        self.references = self.process_references(references)
    ...
    def retrieve_verse(self):
        verse_list = []

        for newReference, encodedReferences in self.references:
            verse_dict = {}
            verses = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--string", nargs='*', help="Input reference string (either this or `-f` flag must be used)")
    ...
    if args.change_language:
        ...
        For usage one of the optional flag is essential [-s] or [-f] along with the language you want output to be in. This script takes in a single reference or a file containing multiple references - each reference in a single line. It also takes in the name of the language in which you want the output and either displays the verses with the reference or writes it to a file for storing
        ...
        -l Specify in which language you want the output. If no language is specified, it defaults to your preferred language. If no language has been set, it defaults to tamil
        ...
        >This reads all the references from my_references.txt and ouputs the verses in Telugu into the file "my_verses.txt"
        ...
        for _lang_ in args.lang:
            ...
            if args.string:
                my_output = GetVerses(lang=_lang_, references = args.string).retrieve_verse()
                ...
                my_output = (GetVerses(lang=_lang_, references=my_refs).retrieve_verse())
                my_final_output.append(my_output)
